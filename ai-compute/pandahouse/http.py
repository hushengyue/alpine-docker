...
def execute(query, connection=None, data=None, external=None, stream=False, timeout=None):
    host, params, files = prepare(query, connection, external=external)

    if timeout != 0:
        params["max_execution_time"] = timeout
        data = params.pop("query").encode("utf-8")
    else:
        database = escape(connection['database'])
        params["query"] = params["query"].format(db=database)
    response = requests.post(host, params=params, data=data,
                             stream=stream, files=files, timeout=timeout+20)
